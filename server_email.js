...
.then(() => {
    process.stdout.write('[EMAIL SERVER] Conectado a MongoDB\n');
    mongoose.connection.db.collection('tickets').createIndex({ siteId: 1, createdAt: -1, _id: -1 })
        .then(() => process.stdout.write('[EMAIL SERVER] Indice de tickets creado\n'))
        .catch(e => process.stdout.write('[EMAIL SERVER] Error creando indice: ' + e.message + '\n'));
})
...
// GET /api/ticket/single/:id - usado por sign.html para cargar ticket
app.get('/api/ticket/single/:id', async (req, res) => {
    try {
        const ticket = await dbCollection('tickets').findOne(idFilter(req.params.id));
        if (!ticket) return res.status(404).json({ error: 'Ticket no encontrado' });
        if (ticket.firmaCliente) {
            return res.json({ alreadySigned: true, descargasRestantes: ticket.descargasRestantes ?? 3 });
        }
        res.json(publicDocument(ticket));
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST /api/ticket/single/:id/sign - guardar firma desde sign.html
app.post('/api/ticket/single/:id/sign', express.json({limit:'10mb'}), async (req, res) => {
    try {
        const { firma, clientName, clientRole } = req.body;
        const patch = { firmaCliente: firma, nombreFirmaCliente: clientName, puestoFirmaCliente: clientRole, estado: 'cerrado', descargasRestantes: 3, fechaFirma: new Date(), updatedAt: new Date() };
        const result = await dbCollection('tickets').findOneAndUpdate(idFilter(req.params.id), { $set: patch }, { returnDocument: 'after' });
        const updated = resultValue(result);
        if (!updated) return res.status(404).json({ error: 'Ticket no encontrado' });
        emitTicket('ticket_updated', updated);
        res.json({ success: true, descargasRestantes: 3 });
    } catch (e) { res.status(500).json({ error: e.message }); }
});

// POST /api/ticket/single/:id/download-pdf - descontar y enviar datos a sign.html
app.post('/api/ticket/single/:id/download-pdf', async (req, res) => {
    try {
        const ticket = await dbCollection('tickets').findOne(idFilter(req.params.id));
        if (!ticket) return res.status(404).json({ error: 'Ticket no encontrado' });
        if ((ticket.descargasRestantes ?? 3) <= 0) return res.status(403).json({ error: 'Límite de descargas alcanzado' });
        const newCount = (ticket.descargasRestantes ?? 3) - 1;
        await dbCollection('tickets').updateOne(idFilter(req.params.id), { $set: { descargasRestantes: newCount } });
...
// START
// ─────────────────────────────────────────────────────────────────────────────
// Escucha en 0.0.0.0 para ser accesible desde Render (producción).
// En local, process.env.PORT no existe, así que usa EMAIL_SERVER_PORT=4010.
server.listen(PORT, '0.0.0.0', () => {
    const isRender = !!process.env.RENDER;
    const url = isRender
        ? `https://server-respaldo-email.onrender.com`
        : `http://localhost:${PORT}`;
    process.stdout.write(`[EMAIL SERVER] ✅ Servidor de correo corriendo en ${url}\n`);
});
